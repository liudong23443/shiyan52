                    ...
                    with st.spinner("正在生成SHAP解释图..."):
                        ...
                        try:
                            ...
                            for text in ax.texts:
                                # 使用系统支持的字体
                                text.set_fontfamily(font_list[0] if font_list else 'sans-serif')
                            
                            # 修改Y轴标签字体
                            for tick in ax.get_yticklabels():
                                tick.set_fontfamily(font_list[0] if font_list else 'sans-serif')
                            
                            # 手动添加标题
                            plt.title("特征对预测的影响", fontsize=12, fontfamily=font_list[0] if font_list else 'sans-serif', weight='bold')
                        
                        except Exception as shap_plt_error:
                            st.warning(f"使用SHAP绘图出错，将使用自定义方法: {str(shap_plt_error)}")
                            plt.close()  # 关闭失败的图表
                            
                            # 创建新图表 - 自定义绘制简单的条形图
                            plt.figure(figsize=(6.5, 4), dpi=200, facecolor='#f8f9fa')
                            
                            # 获取特征重要性
                            feature_names = features_df.columns
                            feature_importance = pd.DataFrame({
                                '特征': feature_names,
                                'SHAP值': shap_obj.values if hasattr(shap_obj, 'values') else shap_obj
                            })
                            
                            # 按绝对值排序
                            feature_importance['绝对值'] = np.abs(feature_importance['SHAP值'])
                            feature_importance = feature_importance.sort_values('绝对值', ascending=False).head(7)
                            
                            # 创建颜色列表
                            colors = ['#2196F3' if val < 0 else '#F44336' for val in feature_importance['SHAP值']]
                            
                            # 绘制条形图
                            ax = plt.barh(feature_importance['特征'], 
                                        feature_importance['SHAP值'], 
                                        color=colors)
                            
                            plt.axvline(x=0, color='#777777', linestyle='-', linewidth=0.8)
                            plt.title("特征对预测的影响", fontsize=12, fontfamily=font_list[0] if font_list else 'sans-serif', weight='bold')
                            plt.xlabel("SHAP值 (负值=降低风险, 正值=增加风险)", fontfamily=font_list[0] if font_list else 'sans-serif')
